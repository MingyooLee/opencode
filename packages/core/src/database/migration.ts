  ...
  return Object.values(desired.tables).flatMap<Operation>((table) => {
    ...
    return [
      ...
      ...Object.values(table.indexes)
        .flatMap((index) => {
          const currentIndex = current.indexes[index.name]
          if (currentIndex === undefined) return [createIndexOperation(index)]
          if (sameIndex(currentIndex, index)) return []
          return [dropIndexOperation(index.name), createIndexOperation(index)]
        }),
    ]
  })
}
...
function dropIndexOperation(index: string): Operation {
  return { type: "drop_index", index }
}
...
function sameIndex(actual: IndexAst, desired: IndexAst) {
  return (
    actual.unique === desired.unique &&
    actual.where === desired.where &&
    actual.columns.length === desired.columns.length &&
    actual.columns.every((column, index) => sameIndexColumn(column, desired.columns[index]))
  )
}

function sameIndexColumn(actual: IndexColumnAst, desired: IndexColumnAst | undefined) {
  if (desired === undefined) return false
  if (actual.type !== desired.type) return false
  if (actual.type === "column" && desired.type === "column") return actual.name === desired.name
  if (actual.type === "expression" && desired.type === "expression") return normalizeSql(actual.sql) === normalizeSql(desired.sql)
  return false
}
...
function toSql(operation: Operation) {
  ...
  if (operation.type === "drop_index") {
    return `DROP INDEX ${quoteIdentifier(operation.index)}`
  }
  return [
    "CREATE",
  ...
  return Effect.gen(function* () {
    ...
    const indexEntries = yield* Effect.forEach(indexes, (index) =>
      Effect.gen(function* () {
        ...
        const expressions = parseIndexColumns(statement.sql)
        ...
        return [
          index.name,
          {
            ...
            columns: columns.map((column) =>
              column.name === null
                ? ({ type: "expression", sql: expressions[column.seqno] ?? "" } as const)
                : ({ type: "column", name: column.name } as const),
            ),
            unique: index.unique === 1,
            ...parseIndexWhere(statement.sql),
          },
        ] as const
      }),
    )
...
function parseIndexColumns(statement: string) {
  ...
  let current = ""
  for (let i = start + 1; i < statement.length; i++) {
...
function parseIndexWhere(statement: string) {
  const match = /\swhere\s/i.exec(statement)
  if (!match) return {}
  return { where: statement.slice(match.index + match[0].length).trim() }
}
