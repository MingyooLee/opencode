...
describe("DatabaseMigration", () => {
  ...
  test("diff is additive only and ignores extra actual tables, columns, indexes, and type drift", () =>
    withDb((db) =>
      Effect.gen(function* () {
        yield* db.run(`CREATE TABLE "session" ("id" text PRIMARY KEY, "title" integer, "extra" text)`)
  ...
  test("diff recreates changed indexes", () =>
    withDb((db) =>
      Effect.gen(function* () {
        yield* db.run(`CREATE TABLE "session" ("id" text PRIMARY KEY, "title" text, "slug" text)`)
        ...
        yield* DatabaseMigration.apply(db, operations)
        expect(yield* DatabaseMigration.diff(db, [table])).toEqual([])
        expect((yield* indexColumns(db, "session_lookup_idx")).map((column) => column.name)).toEqual(["title", "slug"])
      }),
    ),
  )
  ...
  test("random schema reconciliation reaches a fixed point and handles changed indexes", () =>
    withDb((db) =>
      Effect.gen(function* () {
        for (let seed = 1; seed <= 75; seed++) {
          ...
          expect(
            operations.every((operation) => ["create_table", "add_column", "drop_index", "create_index"].includes(operation.type)),
          ).toBe(true)
